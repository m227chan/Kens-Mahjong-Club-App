...
export function MahjongTileSymbol({ tile }: { tile: TileDefinition }) {
  if (tile.kind === 'honor') {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      bodies.current.forEach((body, index) => {
        ...
        const top = body.y - tile.size * 0.59
    ...
    const isProtectedContent = (event: PointerEvent) => {
      ...
      return (
        ...
        Boolean(
          target.closest(
            'button,a,input,select,textarea,[role="button"],[role="link"],label,.club-drag-handle'
          )
        )
      )
    }
    ...
    const hitTest = (point: { x: number; y: number }) => {
      ...
      bodies.current.forEach((body, index) => {
        ...
        if (distance <= body.radius * 1.35 && distance < nearestDistance) {
          match = index
    ...
    const movePointerDrag = (event: PointerEvent) => {
      ...
      if (!drag) {
        document.documentElement.style.cursor =
          !isProtectedContent(event) && hitTest(point) >= 0 ? 'grab' : previousRootCursor
        return
      }
      ...
      if (elapsedSeconds > 0) {
        const instantVx = clamp((point.x - drag.lastX) / elapsedSeconds, -2200, 2200)
        const instantVy = clamp((point.y - drag.lastY) / elapsedSeconds, -2200, 2200)
        drag.vx = drag.vx * 0.3 + instantVx * 0.7
        drag.vy = drag.vy * 0.3 + instantVy * 0.7
      }
      ...
      spins.current[drag.index] = clamp(drag.vx * 0.025, -36, 36)
    }
    ...
    const endPointerDrag = (event: PointerEvent) => {
      ...
      const body = bodies.current[drag.index]
      if (body) {
        const timeSinceLastMove = (event.timeStamp - drag.lastTime) / 1000
    ...
    const tick = (time: number) => {
      ...
      nextBodies.forEach((body, index) => {
        ...
        const angle = speed > 0.01 ? Math.atan2(body.vy, body.vx) : index * 2.39996
      ...
      nextBodies.forEach((body, index) => {
        spins.current[index] = spins.current[index] * Math.exp(-0.5 * elapsedSeconds) + body.vx * 0.002

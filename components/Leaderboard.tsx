...
export function LeaderboardPanel({ clubId, seasonNumber, compact = false, players: suppliedPlayers }: { clubId: string; seasonNumber?: number; compact?: boolean; players?: PlayerDoc[] }) {
  ...
  return (
    <section data-tour="leaderboard" className="leaderboard-board overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      {filtersOpen ? <div className="leaderboard-filter-grid grid gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold text-slate-600">Direction<select value={sortDirection} onChange={(event) => setSortDirection(event.target.value as SortDirection)} className="mt-1 block min-h-10 w-full rounded border border-slate-300 bg-white py-2 pl-3 pr-8 text-sm font-medium text-slate-800"><option value="desc">Descending</option><option value="asc">Ascending</option></select></label>

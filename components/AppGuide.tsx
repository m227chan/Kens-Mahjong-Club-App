...
const GUIDE_SECTIONS = [
  ...
  { icon: '📅', title: 'Seasons and tournaments', body: 'The selector is view-only for everyone: use it to inspect any season, tournament, or all-time club history without changing what the club is playing. Managers choose the shared current competition in Settings, and every member moves to it automatically. A club can have only one unfinished tournament at a time; each tournament keeps separate scores, ratings, standings, and its own clock (24 hours by default). Managers can change, restart, end, or delete tournaments from Settings.' },
  ...
  { icon: '🔢', title: 'Number of Tables', body: 'Set between 1 and 99 tables for the live session. Each table seats exactly 4 players. On mobile, swipe the numbered quick-select strip horizontally to reach tables that are off screen.' },
  ...
  { icon: '🔀', title: 'Shuffle tables', body: 'From the session menu, open Shuffle tables to reseat every full table of 4. Partial tables and the sideline stay put. Choose a mode—Full Random (default), Shark Tank vs Redemption (top vs bottom session scorers), Nemesis (tightest historical point differentials), Never Met (fewest shared games), Skill Balance, or Standings Balance—then Preview the new seating. In the After column, tap two players to swap them before you Confirm. Nothing is saved until you confirm.' },
  { icon: '🎴', title: 'Recording a Game', body: 'Once a table has 4 players it shows ✓ Ready. After a game finishes, tap Winner... to record who won, the win type (self-draw or discard), and the fan count. Use Calculate fan to open the score calculator and Apply fan when the total meets your club minimum. Or tap Draw if no one won.' },
  { icon: '🧭', title: 'Focused wind tracker', body: 'Open a table in focused view for phone scoring. Wind layout (default) asks who starts as East, then tracks round wind, dealer, hand number, and seat winds for everyone at the table. Club settings choose when winds rotate after a win or draw. Use the settings control to switch to the basic seat grid, or restart winds to East after a seat change.' },
  ...
  { icon: '📊', title: 'Score Calculator', body: 'Calculate score: enter the finished tile bag; melds group automatically when the hand is valid—mark concealed/open or swap grouping if needed. Hand helper: pick a pattern you want, then trim and lock tiles toward that hand with useful vs clear-drop highlights. Open it from Club tools, the wiki, or Calculate fan while recording a win. In wind-aware focused view, seat and round winds can prefill from the live table.' },
  ...
  { icon: '⏱️', title: 'Session tracker', body: 'The Player deep dive inside Analytics starts with all recorded games. You can narrow it to the last 24 hours, 48 hours, 7 days, or a custom From/To date range. Use the game-by-game breakdown and press Float to pin a compact chip on this club’s pages.' },
  ...
  { icon: '🔐', title: 'Club settings and managers', body: 'Club Settings groups competition, house scoring, activity, titles, and access into clear categories. Desktop keeps the selected detail beside the list; mobile opens one detail at a time. Managers can use proportional title bands or exact top/bottom counts. Manager-only actions stay hidden or disabled for regular members.' }
] as const
...
const TOUR_STEPS: TourStep[] = [
  ...
  { selector: '[data-tour="season-selector"]', title: 'View competition history', body: 'Use this view-only selector to review regular seasons, ended or current tournaments, or all-time club history. A green dot marks the active season, red marks the current running tournament, and grey marks read-only history. Only manager actions in Settings change the competition for the whole club.', action: 'next' },
  { selector: '[data-tour="club-tools-toggle"]', title: 'Open Club tools', body: 'The Club tools rail starts compact so it never covers your tables or charts. Expand it to reveal Roster, Analytics, Game logs, Network, Score Calculator, and Club settings. On mobile it opens as a temporary drawer.', action: 'next', instruction: 'Expand Club tools, then choose Next.' },
  ...
  { selector: '[data-tour="analytics-open"]', title: 'Open player analytics', body: 'The short-window Session tracker now lives inside the Player deep dive, so there is one place for player performance.', action: 'click', instruction: 'Click Analytics.' },
  { selector: '[data-tour="analytics-modal"]', title: 'Session tracking in context', body: 'Open Player deep dive to use 24-hour, 48-hour, 7-day, or custom date windows, review individual games, and float the tracker.', action: 'next' },
  { selector: '[data-tour="analytics-close"]', title: 'Return to the workspace', body: 'Close Analytics to continue.', action: 'click', instruction: 'Click Close.' },
  ...
  { selector: '[data-tour="analytics-modal"]', title: 'Explore club and player trends', body: 'Switch between the focused Club overview and Player deep dive. Player analytics includes point progression, win types, short-window session totals, game breakdown, and the floating tracker.', action: 'next' },
  ...
  { selector: '[data-tour="network-close"]', title: 'Return to the workspace', body: 'Close the network to continue exploring club tools.', action: 'click', instruction: 'Click Close.' },
  { selector: '[data-tour="score-calculator-open"]', title: 'Open the score calculator', body: 'Score a finished hand with the tile bag, or use Hand helper to shape mid-hand tiles toward a pattern you choose.', action: 'click', instruction: 'Click Score Calculator.' },
  { selector: '[data-tour="score-calculator-modal"]', title: 'Calculate fan while you play', body: 'Use Calculate score for a finished hand. Use Hand helper to pick a target pattern, lock open claims, and see what to keep vs clear-drop. Ming will not change your inputs or record a game.', action: 'next' },
  { selector: '[data-tour="score-calculator-close"]', title: 'Return to the workspace', body: 'Close the score calculator to continue.', action: 'click', instruction: 'Click Close.' },
  ...
  { selector: '[data-tour="settings-modal"]', title: 'You know the core workflow', body: 'Choose a Settings category to manage the one active season, tournaments, club scoring, activity, titles, or access. Sensitive actions stay isolated in Club Access. Ming never clicks those actions, and this tour has made no data writes.', action: 'finish', instruction: 'Finish returns you safely to your dashboard.' }
]
...
export default function AppGuide() {
  ...
  const goBack = useCallback(() => {
    ...
    if (stepIndex === 10) clickTourElement('analytics-close')
    if (stepIndex === 12) clickTourElement('analytics-open')
    ...
    if (stepIndex === 25) clickTourElement('score-calculator-close')
    if (stepIndex === 29) clickTourElement('settings-close')

...
- **Focused table mode.** Open a phone-friendly scorekeeping view for one table. The default wind tracker shows round wind, dealer, hand number, and seat winds (synced for everyone at the table); switch back to the basic seat grid anytime. Print permanent signed QR codes so players can check in at the physical table.
- **Score Calculator.** Build melds, set roll position and flowers, stack handbook patterns into fan totals, and use **Hand helper** to shape a mid-hand toward a pattern you choose. Open it from the club sidebar, wiki, or the **Calculate fan** shortcut while recording a win.
...
- Manager tools for selecting one active season and one unfinished tournament at a time, configurable tournament clocks (24 hours by default), early tournament endings, rosters, memberships, permissions, scoring rules, wind rotation, titles, and safe deletion

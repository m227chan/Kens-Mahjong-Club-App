...
const grantId = (club: string, email: string) =>
  createHash('sha256').update(`${club}:${email}`).digest('hex')
        ...
        action === 'createGame'
          ...
          : action === 'importGames'
            ? {
                ...
                games: body.input?.games,
              }
            : {
                callerUid: caller.uid,
                clubId: body.mutation?.clubId,
                action: body.mutation?.action as
                  | 'update'
                  | 'delete'
                  | 'rebuild',
                gameId: body.mutation?.gameId,
                game: body.mutation?.game,
              }
      const result = await mutateSupabaseGames(input)
      return NextResponse.json({
        result: action === 'createGame' ? result.gameId : undefined,
      })
    }
    const result = await withTransaction(async (db) => {
      await db.query("select set_config('app.actor_uid', $1, true)", [
        caller.uid,
      ])
      const requireManager = async (club: string) => {
        const member = await db.query(
          "select 1 from club_members where club_id=$1 and firebase_uid=$2 and active and role='manager'",
          [club, caller.uid],
        )
        if (!member.rowCount)
          throw new Error('Only an active club manager can do that.')
      }
      ...
      if (action === 'createClub') {
        ...
        if (hasReachedCreatedClubLimit(createdCount))
          throw new Error(CREATED_CLUB_LIMIT_MESSAGE)
        let id = clubId()
        while (
          (await db.query('select 1 from clubs where id=$1', [id])).rowCount
        )
          id = clubId()
        await db.query(
          'insert into clubs(id,name,manager_uid,manager_email,manager_display_name) values($1,$2,$3,$4,$5)',
          [id, name, caller.uid, caller.email ?? null, caller.name ?? null],
        )
        await db.query(
          "insert into club_members(club_id,firebase_uid,email,display_name,photo_url,role) values($1,$2,$3,$4,$5,'manager')",
          [
            id,
            caller.uid,
            caller.email ?? null,
      ...
      if (action === 'resolveJoinRequest') {
        ...
        const requestUid = String(body.request?.uid ?? '')
        const requestRow = (
          await db.query(
            `update join_requests set status=$1,resolved_at=now(),resolved_by=$2
             where club_id=$3 and firebase_uid=$4 and status='pending'
             returning firebase_uid,email,display_name,photo_url`,
            [
              body.approved ? 'approved' : 'declined',
              caller.uid,
              body.clubId,
              requestUid,
            ],
        ...
        if (!requestRow)
          throw new Error('That join request is no longer pending.')
        if (body.approved)
          await db.query(
            `insert into club_members(club_id,firebase_uid,email,display_name,photo_url,role,active) values($1,$2,$3,$4,$5,'member',true)
          on conflict(club_id,firebase_uid) do update set email=excluded.email,display_name=excluded.display_name,photo_url=excluded.photo_url,active=true`,
            [
              body.clubId,
              requestRow.firebase_uid,
              requestRow.email,
              requestRow.display_name,
      ...
      if (action === 'leaveClub') {
        ...
        const member = await db.query(
          ...
          [body.clubId, caller.uid],
        )
        if (member.rows[0]?.role === 'manager')
          throw new Error('Club managers cannot leave their club yet.')
        await db.query(
          'update club_members set active=false where club_id=$1 and firebase_uid=$2',
          [body.clubId, caller.uid],
        )
        return null
      }
      if (action === 'promoteManagerByEmail') {
        ...
        const club = (
          await db.query('select name,universal from clubs where id=$1', [
            body.clubId,
          ])
        ).rows[0]
        try {
          ...
          await db.query(
            `insert into club_members(club_id,firebase_uid,email,display_name,photo_url,role,active,universal) values($1,$2,$3,$4,$5,'manager',true,$6)
          on conflict(club_id,firebase_uid) do update set role='manager',active=true,email=excluded.email,display_name=excluded.display_name,photo_url=excluded.photo_url`,
            [
              body.clubId,
              target.uid,
              target.email ?? null,
          ...
          if ((error as { code?: string }).code !== 'auth/user-not-found')
            throw error
          await db.query(
            `insert into pending_manager_grants(id,club_id,club_name,email_normalized,status,requested_by) values($1,$2,$3,$4,'pending',$5)
          on conflict(id) do update set status='pending',requested_at=now(),requested_by=excluded.requested_by`,
            [
              grantId(body.clubId, email),
              body.clubId,
              club.name,
              email,
              caller.uid,
      ...
      if (action === 'createPlayer') {
        ...
        const authUid = input.authUid == null ? null : String(input.authUid)
        if (authUid && authUid !== caller.uid)
          throw new Error('You can only link a new player to your own account.')
        const icon = String(input.icon ?? name[0] ?? '🀄')
            .trim()
            .slice(0, 12),
          key = encodeURIComponent(icon.toLocaleLowerCase()),
          id = rowId()
        try {
          await db.query(
      ...
      if (action === 'removePlayer') {
        await requireManager(body.clubId)
        await db.query(
          'update players set active=false where id=$1 and club_id=$2',
          [body.playerId, body.clubId],
        )
        return null
      }
      if (action === 'setPlayerAuthLink') {
        ...
        const player = (
          await db.query(
            'select auth_uid from players where id=$1 and club_id=$2 and active',
            [body.playerId, body.clubId],
          )
        ).rows[0]
        if (!player) throw new Error('Player not found.')
        if (!body.linked && player.auth_uid !== caller.uid)
          throw new Error('You can only unlink your own player profile.')
        if (body.linked) {
          if (body.uid !== caller.uid)
            throw new Error('You can only link your own account.')
          if (player.auth_uid && player.auth_uid !== caller.uid)
            throw new Error('That player is already linked to another user.')
          if (
            (
              await db.query(
                'select 1 from players where club_id=$1 and auth_uid=$2 and active and id<>$3',
                [body.clubId, caller.uid, body.playerId],
              )
            ).rowCount
          )
            throw new Error(
              'Your account is already linked to another player in this club.',
            )
        }
        await db.query(
          'update players set auth_uid=$1 where id=$2 and club_id=$3',
          [body.linked ? caller.uid : null, body.playerId, body.clubId],
        )
        return null
      }
      ...
      if (action === 'ensureSeasons') {
        await requireMember(body.clubId)
        await db.query(
          "insert into seasons(club_id,season_number,name,created_by) values($1,1,'Season 1',$2) on conflict do nothing",
          [body.clubId, caller.uid],
        )
        return null
      }
      ...
      if (action === 'createSession') {
        ...
        if (validPlayers.rowCount !== new Set(participants).size)
          throw new Error(
            'One or more selected players are no longer on the roster. Refresh and try again.',
          )
        await db.query('select pg_advisory_xact_lock(hashtext($1))', [
          `session:${body.clubId}`,
        ])
        const existing = (
          await db.query(
            'select id,season_number from sessions where club_id=$1 and is_active for update',
            [body.clubId],
          )
        ).rows[0]
        const layout = normalizeSessionLayout(
          participants,
          tableCount,
          input.tables && typeof input.tables === 'object' ? input.tables : {},
          Array.isArray(input.sideline)
            ? input.sideline.map(String)
            : participants,
        )
        const { tables, sideline } = layout
        if (existing?.season_number === seasonNumber) {
          await db.query(
            'update sessions set table_count=$1,participants=$2,tables=$3,sideline=$4 where id=$5',
            [
              tableCount,
              participants,
        ...
        const created = await db.query(
          'insert into sessions(club_id,created_by,season_number,table_count,participants,tables,sideline) values($1,$2,$3,$4,$5,$6,$7) returning id',
          [
            body.clubId,
            caller.uid,
            seasonNumber,

...
type Exchange = {
  ...
  clubName: string
  tableNumber: number
  enrollmentStatus?: 'required' | 'pending'
  linkedPlayer: TablePlayer | null
  players: TablePlayer[]
  unlinkedPlayers: TablePlayer[]
  checkIn?: CheckIn
}
...
export default function TableCheckInPage() {
  ...
  const finishCheckIn = async (details: Exchange, replacePlayerId?: string) => {
    const result = await tableAction<CheckIn>({
      action: 'checkIn',
    ...
    router.replace(
      `/club/${encodeURIComponent(details.clubId)}/table/${details.tableNumber}`,
    )
  }
  ...
  useEffect(() => {
    ...
    setBusy(true)
    setError(null)
    void tableAction<Exchange>({ action: 'exchange', publicId, signature })
      .then(async (result) => {
        setExchange(result)
        setNewName(user.displayName ?? '')
        if (result.checkIn?.status === 'table_full')
          setFullTable(result.checkIn.occupants)
        else if (result.checkIn?.status === 'ok') {
          window.sessionStorage.removeItem(`mahjong:table-qr:${publicId}`)
          router.replace(
            `/club/${encodeURIComponent(result.clubId)}/table/${result.tableNumber}`,
          )
        } else if (result.linkedPlayer) await finishCheckIn(result)
      })
      .catch((nextError) =>
        setError(
  ...
  const filtered = useMemo(
    () =>
      exchange?.unlinkedPlayers.filter((player) =>
        player.displayName
          .toLocaleLowerCase()
  ...
  const requestEnrollment = async () => {
    ...
    setError(null)
    try {
      const result = await tableAction<{
        enrollmentStatus: 'pending' | 'member' | 'retry'
      }>({ action: 'requestEnrollment', publicId, signature })
      if (
        result.enrollmentStatus === 'member' ||
        result.enrollmentStatus === 'retry'
      ) {
      ...
      setExchange((current) =>
        current ? { ...current, enrollmentStatus: 'pending' } : current,
      )
    ...
    <main className="mx-auto flex min-h-[calc(100dvh-90px)] max-w-lg items-center px-4 py-8">
      <section className="w-full rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
        ...
        {error || authError ? (
          <p
            role="alert"
            className="mt-4 rounded border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-800"
          >
            {error ?? authError}
          </p>
        ) : null}

        {exchange?.enrollmentStatus && !busy ? (
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-slate-900">
            <h2 className="text-lg font-black">
              {exchange.enrollmentStatus === 'pending'
                ? 'Waiting for manager approval'
                : `Request to join ${exchange.clubName}`}
            </h2>
            <p className="mt-2 text-sm text-slate-700">
              {exchange.enrollmentStatus === 'pending'
                ? 'Your request has been sent. Once a manager approves it, come back here to continue linking your roster player and join the table.'
                : 'This club requires a manager to approve new members before a table QR can check them in.'}
            </p>
            {exchange.enrollmentStatus === 'required' ? (
              <button
                type="button"
              ...
              <button
                ...
                className="mt-4 min-h-12 w-full rounded-lg border border-amber-300 bg-white px-4 font-black text-slate-900"
              >
        ...
        {exchange &&
        !exchange.enrollmentStatus &&
        !exchange.linkedPlayer &&
        !fullTable &&
        !busy ? (
          <div className="mt-5 space-y-5">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Link your account to an existing roster player, or create
                yourself once.
              </p>
            </div>
            <input
            ...
            <div className="max-h-56 space-y-2 overflow-y-auto">
              {filtered.map((player) => (
                <button
                  ...
                  onClick={() =>
                    window.confirm(
                      `Link your account to ${player.displayName}?`,
                    ) && void resolvePlayer('link', player.id)
                  }
                  className="flex min-h-12 w-full items-center gap-3 rounded-lg border border-slate-200 px-3 text-left font-bold"
                >

...
begin
  ...
  for sole_club in
    ...
    where c.active
      and not exists (
        select 1
        from public.club_members other_manager
  ...
  -- If another manager already existed, make that manager the displayed owner
  -- before removing the departing manager's membership.
  for sole_club in
    select c.id
    from public.clubs c
  ...
  -- Preserve club history while removing the deleted account's identity links.
